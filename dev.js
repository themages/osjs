...
var xml2json$1 = function (xmlString) {
  return xmlToJSON.parseString(xmlString);
};

var xml2json_1 = xml2json$1;
...
var json2xml$1 = function (obj, options) {
  if (!options) {
    options = {
      xmlHeader: {
        standalone: true
...
var CryptoJS = crypto.exports;
var xml2json = xml2json_1;
...
var getAuth$1 = function (opt) {
  ...
  var formatString = [method, pathname, util$5.obj2str(queryParams, true), util$5.obj2str(headers, true), ''].join('\n'); // 步骤三：计算 StringToSign
...
var parseSelectPayload = function (chunk) {
  ...
  while (chunk.byteLength) {
    ...
    if (header[':event-type'] === 'Records') {
      ...
      content = buf2str(chunk, offset, offset + payloadRestLength, true);
      result.stats = util$5.xml2json(content).Stats;
...
var getBodyMd5 = function (UploadCheckContentMd5, Body, callback, onProgress) {
  ...
  if (UploadCheckContentMd5) {
    if (typeof Body === 'string') {
      callback(util$5.md5(Body, true));
    } else if (Blob && Body instanceof Blob) {
      util$5.getFileMd5(Body, function (err, md5) {
        callback(md5);
      }, onProgress);
    } else {
      callback();
...
var getFileMd5 = function (blob, callback, onProgress) {
  ...
  var next = function (start) {
    ...
    util$5.fileSlice(blob, start, end, false, function (chunk) {
...
var formatParams = function (apiName, params) {
  ...
  if (apiName !== 'getAuth' && apiName !== 'getV4Auth' && apiName !== 'getObjectUrl') {
    ...
    if (params && typeof params === 'object') {
      ...
      util$5.each(headerMap, function (paramKey, headerKey) {
  ...
  return function (params, callback) {
    ...
    if (window.Promise && !isSync && !callback) {
      return new Promise(function (resolve, reject) {
        ...
        if (errMsg) return _callback(util$5.error(new Error(errMsg)));
...
var getFileSize = function (api, params, callback) {
  ...
  if (params.Body && (params.Body instanceof Blob || params.Body.toString() === '[object File]' || params.Body.toString() === '[object Blob]')) {
    ...
    callback(util$5.error(new Error('params body format error, Only allow File|Blob|String.')));
    return;
  }
...
var util$4 = util_1; // 按照文件特征值，缓存 UploadId
...
var mod = {
  ...
  getFileId: function (file, ChunkSize, Bucket, Key) {
    if (file.name && file.size && file.lastModifiedDate && ChunkSize) {
      return util$4.md5([file.name, file.size, file.lastModifiedDate, ChunkSize, Bucket, Key].join('::'));
    } else {
      return null;
    }
  },
...
var initTask = function (cos) {
  ...
  var emitListUpdate = function () {
    ...
    var emit = function () {
      ...
      cos.emit('task-list-update', {
        list: util$3.map(queue, formatTask)
      });
  ...
  var startNextTask = function () {
    ...
    var apiParams = util$3.formatParams(task.api, task.params);
  ...
  var killTask = function (id, switchToState) {
    ...
    if (switchToState === 'canceled' && task.state !== 'canceled' || switchToState === 'paused' && waiting || switchToState === 'paused' && running) {
      ...
      if (switchToState === 'canceled' && UploadId) session$1.removeUsing(UploadId);
      emitListUpdate();
  ...
  cos._addTasks = function (taskList) {
    util$3.each(taskList, function (task) {
  ...
  cos._addTask = function (api, params, callback, ignoreAddEvent) {
    // 复制参数对象
    params = util$3.formatParams(api, params); // 生成 id
    ...
    util$3.getFileSize(api, params, function (err, size) {
      // 开始处理上传
      if (err) return callback(util$3.error(err)); // 如果获取大小出错，不加入队列
      // 获取完文件大小再把任务加入队列
  ...
  cos.getTaskList = function () {
    return util$3.map(queue, formatTask);
  };
...
function getService(params, callback) {
  ...
  var protocol = this.options.Protocol || (util$2.isBrowser && location.protocol === 'http:' ? 'http:' : 'https:');
  ...
  submitRequest.call(this, {
    ...
    buckets = util$2.isArray(buckets) ? buckets : [buckets];
...
function getBucket(params, callback) {
  ...
  submitRequest.call(this, {
    ...
    Contents = util$2.isArray(Contents) ? Contents : [Contents];
    CommonPrefixes = util$2.isArray(CommonPrefixes) ? CommonPrefixes : [CommonPrefixes];
    var result = util$2.clone(ListBucketResult);
    util$2.extend(result, {
      Contents: Contents,
...
function putBucketAcl(params, callback) {
  ...
  if (params['AccessControlPolicy']) {
    var AccessControlPolicy = util$2.clone(params['AccessControlPolicy'] || {});
    var Grants = AccessControlPolicy.Grants || AccessControlPolicy.Grant;
    Grants = util$2.isArray(Grants) ? Grants : [Grants];
    ...
    headers['Content-MD5'] = util$2.binaryBase64(util$2.md5(xml));
  } // Grant Header 去重


  util$2.each(headers, function (val, key) {
...
function getBucketAcl(params, callback) {
  submitRequest.call(this, {
    ...
    Grant = util$2.isArray(Grant) ? Grant : [Grant];
...
function putBucketCors(params, callback) {
  ...
  CORSRules = util$2.clone(util$2.isArray(CORSRules) ? CORSRules : [CORSRules]);
  util$2.each(CORSRules, function (rule) {
    util$2.each(['AllowedOrigin', 'AllowedHeader', 'AllowedMethod', 'ExposeHeader'], function (key) {
      var sKey = key + 's';
      var val = rule[sKey] || rule[key] || [];
      delete rule[sKey];
      rule[key] = util$2.isArray(val) ? val : [val];
    });
  });
  var xml = util$2.json2xml({
    CORSConfiguration: {
      CORSRule: CORSRules
    }
  });
  ...
  submitRequest.call(this, {
    ...
    util$2.each(CORSRules, function (rule) {
      util$2.each(['AllowedOrigin', 'AllowedHeader', 'AllowedMethod', 'ExposeHeader'], function (key) {
        ...
        var val = rule[sKey] || rule[key] || [];
        delete rule[key];
        rule[sKey] = util$2.isArray(val) ? val : [val];
      });
    });
    callback(null, {
      CORSRules: CORSRules,
...
function putBucketPolicy(params, callback) {
  ...
  if (!Policy || typeof Policy === 'string') return callback(util$2.error(new Error('Policy format error')));
  var PolicyStr = JSON.stringify(Policy);
  if (!Policy.version) Policy.version = '2.0';
  var headers = params.Headers;
  headers['Content-Type'] = 'application/json';
  headers['Content-MD5'] = util$2.binaryBase64(util$2.md5(PolicyStr));
  submitRequest.call(this, {
  ...
  submitRequest.call(this, {
    ...
    if (err) {
      if (err.statusCode && err.statusCode === 403) {
        return callback(util$2.error(err, {
...
function putBucketTagging(params, callback) {
  ...
  Tags = util$2.clone(util$2.isArray(Tags) ? Tags : [Tags]);
  var xml = util$2.json2xml({
    Tagging: {
      TagSet: {
        Tag: Tags
...
function getBucketLifecycle(params, callback) {
  submitRequest.call(this, {
    ...
    Rules = util$2.clone(util$2.isArray(Rules) ? Rules : [Rules]);
    callback(null, {
      Rules: Rules,
      statusCode: data.statusCode,
      headers: data.headers
...
function putBucketVersioning(params, callback) {
  if (!params['VersioningConfiguration']) {
    callback(util$2.error(new Error('missing param VersioningConfiguration')));
    return;
  }
...
function putBucketReplication(params, callback) {
  var ReplicationConfiguration = util$2.clone(params.ReplicationConfiguration);
  var xml = util$2.json2xml({
...
function getBucketReplication(params, callback) {
  submitRequest.call(this, {
    ...
    if (data.ReplicationConfiguration.Rule) {
      data.ReplicationConfiguration.Rules = util$2.makeArray(data.ReplicationConfiguration.Rule);
...
function putBucketWebsite(params, callback) {
  if (!params['WebsiteConfiguration']) {
    callback(util$2.error(new Error('missing param WebsiteConfiguration')));
    return;
  }

  var WebsiteConfiguration = util$2.clone(params['WebsiteConfiguration'] || {});
  var RoutingRules = WebsiteConfiguration['RoutingRules'] || WebsiteConfiguration['RoutingRule'] || [];
  RoutingRules = util$2.isArray(RoutingRules) ? RoutingRules : [RoutingRules];
...
function getBucketWebsite(params, callback) {
  submitRequest.call(this, {
    ...
    if (WebsiteConfiguration['RoutingRules']) {
      var RoutingRules = util$2.clone(WebsiteConfiguration['RoutingRules'].RoutingRule || []);
      RoutingRules = util$2.makeArray(RoutingRules);
...
function putBucketReferer(params, callback) {
  if (!params['RefererConfiguration']) {
    callback(util$2.error(new Error('missing param RefererConfiguration')));
    return;
  }

  var RefererConfiguration = util$2.clone(params['RefererConfiguration'] || {});
  var DomainList = RefererConfiguration['DomainList'] || {};
  var Domains = DomainList['Domains'] || DomainList['Domain'] || [];
  Domains = util$2.isArray(Domains) ? Domains : [Domains];
...
function getBucketReferer(params, callback) {
  submitRequest.call(this, {
    ...
    if (RefererConfiguration['DomainList']) {
      var Domains = util$2.makeArray(RefererConfiguration['DomainList'].Domain || []);
...
function putBucketDomain(params, callback) {
  ...
  DomainRule = util$2.clone(DomainRule);
  var xml = util$2.json2xml({
    DomainConfiguration: {
...
function getBucketDomain(params, callback) {
  submitRequest.call(this, {
    ...
    DomainRule = util$2.clone(util$2.isArray(DomainRule) ? DomainRule : [DomainRule]);
    callback(null, {
      DomainRule: DomainRule,
      statusCode: data.statusCode,
      headers: data.headers
...
function putBucketOrigin(params, callback) {
  ...
  OriginRule = util$2.clone(OriginRule);
  var xml = util$2.json2xml({
    OriginConfiguration: {
...
function getBucketOrigin(params, callback) {
  submitRequest.call(this, {
    ...
    OriginRule = util$2.clone(util$2.isArray(OriginRule) ? OriginRule : [OriginRule]);
    callback(null, {
      OriginRule: OriginRule,
      statusCode: data.statusCode,
      headers: data.headers
...
function putBucketInventory(params, callback) {
  var InventoryConfiguration = util$2.clone(params['InventoryConfiguration']);
...
function listBucketInventory(params, callback) {
  submitRequest.call(this, {
    ...
    InventoryConfigurations = util$2.isArray(InventoryConfigurations) ? InventoryConfigurations : [InventoryConfigurations];
    delete ListInventoryConfigurationResult['InventoryConfiguration'];
    util$2.each(InventoryConfigurations, function (InventoryConfiguration) {
      if (InventoryConfiguration && InventoryConfiguration.OptionalFields && InventoryConfiguration.OptionalFields.Field) {
        var Field = InventoryConfiguration.OptionalFields.Field;
    ...
    util$2.extend(ListInventoryConfigurationResult, {
      statusCode: data.statusCode,
      headers: data.headers
    });
...
function putBucketAccelerate(params, callback) {
  if (!params['AccelerateConfiguration']) {
    callback(util$2.error(new Error('missing param AccelerateConfiguration')));
    return;
  }
  ...
  var xml = util$2.json2xml(configuration);
  var headers = {};
  headers['Content-Type'] = 'application/xml';
  headers['Content-MD5'] = util$2.binaryBase64(util$2.md5(xml));
  submitRequest.call(this, {
...
function getBucketEncryption(params, callback) {
  submitRequest.call(this, {
    ...
    var Rules = util$2.makeArray(data.EncryptionConfiguration && data.EncryptionConfiguration.Rule || []);
...
function headObject(params, callback) {
  submitRequest.call(this, {
    ...
    data.ETag = util$2.attr(data.headers, 'etag', '');
    callback(null, data);
  });
}
...
function listObjectVersions(params, callback) {
  ...
  submitRequest.call(this, {
    ...
    DeleteMarkers = util$2.isArray(DeleteMarkers) ? DeleteMarkers : [DeleteMarkers];
    var Versions = ListVersionsResult.Version || [];
    Versions = util$2.isArray(Versions) ? Versions : [Versions];
    var result = util$2.clone(ListVersionsResult);
...
function getObject(params, callback) {
  ...
  var onProgress = util$2.throttleOnProgress.call(this, 0, params.onProgress);
  ...
  submitRequest.call(this, {
    ...
    callback(null, {
      Body: data.body,
      ETag: util$2.attr(data.headers, 'etag', ''),
      statusCode: data.statusCode,
      headers: data.headers
    });
  });
...
function putObject(params, callback) {
  ...
  var onProgress = util$2.throttleOnProgress.call(self, FileSize, params.onProgress); // 特殊处理 Cache-Control、Content-Type，避免代理更改这两个字段导致写入到 Object 属性里
  ...
  util$2.getBodyMd5(needCalcMd5, params.Body, function (md5) {
    if (md5) {
      if (self.options.UploadCheckContentMd5) headers['Content-MD5'] = util$2.binaryBase64(md5);
...
function putObjectCopy(params, callback) {
  ...
  var m = util$2.getSourceParams.call(this, CopySource);

  if (!m) {
    callback(util$2.error(new Error('CopySource format error')));
    return;
  }
  ...
  submitRequest.call(this, {
    ...
    if (err) return callback(err);
    var result = util$2.clone(data.CopyObjectResult || {});
    var url = getUrl({
      ForcePathStyle: self.options.ForcePathStyle,
      protocol: self.options.Protocol,
      domain: self.options.Domain,
...
function uploadPartCopy(params, callback) {
  ...
  submitRequest.call(this, {
    ...
    if (err) return callback(err);
    var result = util$2.clone(data.CopyPartResult || {});
    util$2.extend(result, {
      statusCode: data.statusCode,
      headers: data.headers
    });
...
function deleteMultipleObject(params, callback) {
  ...
  Objects = util$2.isArray(Objects) ? Objects : [Objects];
  var xml = util$2.json2xml({
    Delete: {
      Object: Objects,
      Quiet: Quiet || false
  ...
  headers['Content-Type'] = 'application/xml';
  headers['Content-MD5'] = util$2.binaryBase64(util$2.md5(xml));
  var Scope = util$2.map(Objects, function (v) {
    return {
      action: 'name/cos:DeleteObject',
      bucket: params.Bucket,
      region: params.Region,
  ...
  submitRequest.call(this, {
    ...
    Deleted = util$2.isArray(Deleted) ? Deleted : [Deleted];
    Errors = util$2.isArray(Errors) ? Errors : [Errors];
    var result = util$2.clone(DeleteResult);
    util$2.extend(result, {
      Error: Errors,
      Deleted: Deleted,
      statusCode: data.statusCode,
...
function restoreObject(params, callback) {
  ...
  if (!params['RestoreRequest']) {
    callback(util$2.error(new Error('missing param RestoreRequest')));
    return;
  }
...
function selectObjectContent(params, callback) {
  ...
  if (!SelectType) return callback(util$2.error(new Error('missing param SelectType')));
  ...
  submitRequest.call(this, {
    ...
    var result = util$2.parseSelectPayload(data.body);
    callback(null, {
      statusCode: data.statusCode,
      headers: data.headers,
      Body: result.body,
...
function multipartInit(params, callback) {
  ...
  if (!headers['Cache-Control'] && !headers['cache-control']) headers['Cache-Control'] = '';
  if (!headers['Content-Type'] && !headers['content-type']) headers['Content-Type'] = params.Body && params.Body.type || '';
  util$2.getBodyMd5(params.Body && (params.UploadAddMetaMd5 || self.options.UploadAddMetaMd5), params.Body, function (md5) {
    ...
    submitRequest.call(self, {
      ...
      if (err) return callback(err);
      data = util$2.clone(data || {});

      if (data && data.InitiateMultipartUploadResult) {
        return callback(null, util$2.extend(data.InitiateMultipartUploadResult, {
          statusCode: data.statusCode,
          headers: data.headers
        }));
      }
...
function multipartUpload(params, callback) {
  var self = this;
  util$2.getFileSize('multipartUpload', params, function () {
    util$2.getBodyMd5(self.options.UploadCheckContentMd5, params.Body, function (md5) {
      if (md5) params.Headers['Content-MD5'] = util$2.binaryBase64(md5);
      submitRequest.call(self, {
...
function multipartList(params, callback) {
  ...
  reqParams = util$2.clearKey(reqParams);
  submitRequest.call(this, {
    ...
    if (data && data.ListMultipartUploadsResult) {
      ...
      Upload = util$2.isArray(Upload) ? Upload : [Upload];
      data.ListMultipartUploadsResult.Upload = Upload;
    }

    var result = util$2.clone(data.ListMultipartUploadsResult || {});
    util$2.extend(result, {
      statusCode: data.statusCode,
      headers: data.headers
    });
...
function multipartListPart(params, callback) {
  ...
  submitRequest.call(this, {
    ...
    Part = util$2.isArray(Part) ? Part : [Part];
    ListPartsResult.Part = Part;
    var result = util$2.clone(ListPartsResult);
    util$2.extend(result, {
      statusCode: data.statusCode,
      headers: data.headers
    });
...
function getObjectUrl(params, callback) {
  ...
  if (params.Query) {
    queryParamsStr += util$2.obj2str(params.Query);
  }
...
function decodeAcl(AccessControlPolicy) {
  ...
  Grant && Grant.length && util$2.each(Grant, function (item) {
  ...
  util$2.each(GrantMap, function (item) {
...
function getUrl(params) {
  ...
  var protocol = params.protocol || (util$2.isBrowser && location.protocol === 'http:' ? 'http:' : 'https:');
  ...
  if (object) {
    url += util$2.camSafeUrlEncode(object).replace(/%2F/g, '/');
  }
...
function getAuthorizationAsync(params, callback) {
  var headers = util$2.clone(params.Headers);
  var headerHost = '';
  util$2.each(headers, function (v, k) {
  ...
  var cb = function (err, AuthData) {
    ...
    if (AuthData && AuthData.XCosSecurityToken && !AuthData.SecurityToken) {
      AuthData = util$2.clone(AuthData);
  ...
  var ScopeKey = util$2.md5(JSON.stringify(Scope)); // STS
  ...
  (function () {
    ...
    for (i = self._StsCache.length - 1; i >= 0; i--) {
      ...
      var compareTime = Math.round(util$2.getSkewTime(self.options.SystemClockOffset) / 1000) + 30;
  ...
  var calcAuthByTmpKey = function () {
    ...
    var Authorization = util$2.getAuth({
  ...
  var checkAuthError = function (AuthData) {
    if (AuthData.Authorization) {
      ...
      if (!formatAllow) return util$2.error(new Error('getAuthorization callback params format error'));
    } else {
      if (!AuthData.TmpSecretId) return util$2.error(new Error('getAuthorization callback params missing "TmpSecretId"'));
      if (!AuthData.TmpSecretKey) return util$2.error(new Error('getAuthorization callback params missing "TmpSecretKey"'));
      if (!AuthData.SecurityToken && !AuthData.XCosSecurityToken) return util$2.error(new Error('getAuthorization callback params missing "SecurityToken"'));
      if (!AuthData.ExpiredTime) return util$2.error(new Error('getAuthorization callback params missing "ExpiredTime"'));
      if (AuthData.ExpiredTime && AuthData.ExpiredTime.toString().length !== 10) return util$2.error(new Error('getAuthorization callback params "ExpiredTime" should be 10 digits'));
      if (AuthData.StartTime && AuthData.StartTime.toString().length !== 10) return util$2.error(new Error('getAuthorization callback params "StartTime" should be 10 StartTime'));
    }
  ...
  if (StsData.ExpiredTime && StsData.ExpiredTime - util$2.getSkewTime(self.options.SystemClockOffset) / 1000 > 60) {
...
function allowRetry(err) {
  ...
  if (err) {
    if (isTimeError && serverDate) {
      ...
      if (this.options.CorrectClockSkew && Math.abs(util$2.getSkewTime(this.options.SystemClockOffset) - serverTime) >= 30000) {
...
function submitRequest(params, callback) {
  ...
  params.qs = util$2.clearKey(params.qs); // 清理 undefined 和 null 字段

  params.headers && (params.headers = util$2.clearKey(params.headers));
  params.qs && (params.qs = util$2.clearKey(params.qs));
  var Query = util$2.clone(params.qs);
...
function _submitRequest(params, callback) {
  ...
  opt.headers && (opt.headers = util$2.clearKey(opt.headers));
  opt = util$2.clearKey(opt); // progress
  ...
  var sender = (self.options.Request || REQUEST)(opt, function (r) {
    ...
    var cb = function (err, data) {
      ...
      if (err) {
        err = util$2.extend(err || {}, attrs);
        callback(err, null);
      } else {
        data = util$2.extend(data || {}, attrs);
        callback(null, data);
      }
    ...
    if (err) return cb(util$2.error(err)); // 请求返回码不为 200
    ...
    try {
      json = body && body.indexOf('<') > -1 && body.indexOf('>') > -1 && util$2.xml2json(body) || {};
    } catch (e) {
      json = {};
    } // 处理返回值
    ...
    if (statusSuccess) {
      ...
      cb(util$2.error(new Error(xmlError.Message), {
        code: xmlError.Code,
        error: xmlError
      }));
    } else if (statusCode) {
      // 有错误的状态码
      cb(util$2.error(new Error(response.statusMessage), {
        code: '' + statusCode
      }));
    } else if (statusCode) {
      // 无状态码，或者获取不到状态码
      cb(util$2.error(new Error('statusCode error')));
    }
  }); // kill task
...
function warnOldApi(apiName, fn, proto) {
  util$2.each(['Cors', 'Acl'], function (suffix) {
    if (apiName.slice(-suffix.length) === suffix) {
      var oldName = apiName.slice(0, -suffix.length) + suffix.toUpperCase();
      var apiFn = util$2.apiWrapper(apiName, fn);
      var warned = false;
...
base$1.init = function (COS, task) {
  task.transferToTaskMethod(API_MAP$1, 'putObject');
  util$2.each(API_MAP$1, function (fn, apiName) {
    COS.prototype[apiName] = util$2.apiWrapper(apiName, fn);
...
var Async = async_1;
var EventProxy = event$1.EventProxy;
var util$1 = util_1; // 文件分块上传全过程，暴露的分块上传接口
...
function sliceUploadFile(params, callback) {
  ...
  ep.on('upload_complete', function (UploadCompleteData) {
    var _UploadCompleteData = util$1.extend({
  ...
  ep.on('upload_slice_complete', function (UploadData) {
    var metaHeaders = {};
    util$1.each(params.Headers, function (val, k) {
  ...
  ep.on('get_file_size_finish', function () {
    onProgress = util$1.throttleOnProgress.call(self, FileSize, params.onProgress);

    if (params.UploadData.UploadId) {
      ep.emit('get_upload_data_finish', params.UploadData);
    } else {
      var _params = util$1.extend({
        TaskId: TaskId,
        Bucket: Bucket,
        Region: Region,
        Key: Key,
  ...
  util$1.each(params.Headers, function (item, key) {
...
function getUploadIdAndPartList(params, callback) {
  ...
  var FinishSize = 0;
  var onHashProgress = util$1.throttleOnProgress.call(self, FileSize, params.onHashProgress);
  ...
  var getChunkETag = function (PartNumber, callback) {
    ...
    if (ETagMap[PartNumber]) {
      ...
      util$1.fileSlice(params.Body, start, end, false, function (chunkItem) {
        util$1.getFileMd5(chunkItem, function (err, md5) {
          if (err) return callback(util$1.error(err));
          var ETag = '"' + md5 + '"';
  ...
  ep.on('upload_id_available', function (UploadData) {
    ...
    var map = {};
    var list = [];
    util$1.each(UploadData.PartList, function (item) {
  ...
  ep.on('no_available_upload_id', function () {
    ...
    var _params = util$1.extend({
      ...
      Region: Region,
      Key: Key,
      Query: util$1.clone(params.Query),
      StorageClass: StorageClass,
      Body: params.Body
    }, params);

    var headers = util$1.clone(params.Headers);
    ...
    self.multipartInit(_params, function (err, data) {
      ...
      if (!UploadId) {
        return callback(util$1.error(new Error('no such upload id')));
      }
  ...
  ep.on('seek_local_avail_upload_id', function (RemoteUploadIdList) {
    ...
    var next = function (index) {
      ...
      if (!util$1.isInArray(RemoteUploadIdList, UploadId)) {
  ...
  ep.on('get_remote_upload_id_list', function () {
    ...
    wholeMultipartList.call(self, {
      ...
      var RemoteUploadIdList = util$1.filter(data.UploadList, function (item) {
      ...
      if (RemoteUploadIdList.length) {
        ...
        if (uuid && (LocalUploadIdList = session.getUploadIdList.call(self, uuid))) {
          util$1.each(LocalUploadIdList, function (UploadId) {
            session.removeUploadId.call(self, UploadId);
          });
        }
...
function uploadSliceList(params, cb) {
  ...
  var ServerSideEncryption = params.ServerSideEncryption;
  var Headers = params.Headers;
  var needUploadSlices = util$1.filter(UploadData.PartList, function (SliceItem) {
...
function uploadSliceItem(params, callback) {
  ...
  var headers = {};
  util$1.each(Headers, function (v, k) {
  ...
  Async.retry(ChunkRetryTimes, function (tryCallback) {
    if (!self._isRunningTask(TaskId)) return;
    util$1.fileSlice(FileBody, start, end, true, function (Body) {
...
function abortUploadTask(params, callback) {
  ...
  if (Level === 'bucket') {
    ...
    if (!Key) return callback(util$1.error(new Error('abort_upload_task_no_key')));
    wholeMultipartList.call(self, {
      Bucket: Bucket,
      Region: Region,
      Key: Key
    ...
    if (!UploadId) return callback(util$1.error(new Error('abort_upload_task_no_id')));
    if (!Key) return callback(util$1.error(new Error('abort_upload_task_no_key')));
    ...
    return callback(util$1.error(new Error('abort_unknown_level')));
  }
} // 批量抛弃分块上传任务
...
function uploadFile(params, callback) {
  ...
  util$1.each(params, function (v, k) {
...
function uploadFiles(params, callback) {
  ...
  var TotalSize = 0;
  var TotalFinish = 0;
  var onTotalProgress = util$1.throttleOnProgress.call(self, TotalFinish, params.onProgress); // 汇总返回回调
  ...
  var taskList = [];
  util$1.each(params.files, function (fileParams, index) {
    (function () {
      // 对齐 nodejs 缩进
      ...
      util$1.each(fileParams, function (v, k) {
        if (typeof v !== 'object' && typeof v !== 'function') {
          fileInfo[k] = v;
        }
      }); // 处理单个文件 TaskReady
...
function sliceCopyFile(params, callback) {
  ...
  var Key = params.Key;
  var CopySource = params.CopySource;
  var m = util$1.getSourceParams.call(this, CopySource);

  if (!m) {
    callback(util$1.error(new Error('CopySource format error')));
    return;
  }
  ...
  ep.on('copy_slice_complete', function (UploadData) {
    ...
    var Parts = util$1.map(UploadData.PartList, function (item) {
      return {
        PartNumber: item.PartNumber,
        ETag: item.ETag
      };
  ...
  ep.on('get_file_size_finish', function (SourceHeaders) {
    ...
    TargetHeader = util$1.clearKey(TargetHeader);
    /**
    ...
    if (SourceHeaders['x-cos-storage-class'] === 'ARCHIVE' || SourceHeaders['x-cos-storage-class'] === 'DEEP_ARCHIVE') {
      ...
      if (!restoreHeader || restoreHeader === 'ongoing-request="true"') {
        callback(util$1.error(new Error('Unrestored archive object is not allowed to be copied')));
        return;
      }
    }
    /**
  ...
  self.headObject({
    ...
    if (FileSize === undefined || !FileSize) {
      callback(util$1.error(new Error('get Content-Length error, please add "Content-Length" to CORS ExposeHeader setting.（ 获取Content-Length失败，请在CORS ExposeHeader设置中添加Content-Length，请参考文档：https://cloud.tencent.com/document/product/436/13318 ）')));
      return;
    }

    onProgress = util$1.throttleOnProgress.call(self, FileSize, params.onProgress); // 开始上传
    ...
    if (FileSize <= CopySliceSize) {
      ...
      util$1.each(resHeaders, function (v, k) {
...
advance$1.init = function (COS, task) {
  task.transferToTaskMethod(API_MAP, 'sliceUploadFile');
  util$1.each(API_MAP, function (fn, apiName) {
    COS.prototype[apiName] = util$1.apiWrapper(apiName, fn);
  });
};
